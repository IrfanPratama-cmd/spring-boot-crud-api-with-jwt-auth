package jwtspringproduct.jwtspringproduct.config;
...
import static jwtspringproduct.jwtspringproduct.model.Role.ADMIN;
import static jwtspringproduct.jwtspringproduct.model.Role.USERS;
...
public class SecurityConfiguration {
  ...
  public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    http
        ...
        .requestMatchers("/api/v1/**").hasAnyRole(ADMIN.name(), USERS.name())


        .requestMatchers(GET, "/api/v1/**").hasAnyAuthority(ADMIN_READ.name(), USER_READ.name())
        .requestMatchers(POST, "/api/v1/**").hasAnyAuthority(ADMIN_CREATE.name(), USER_CREATE.name())
        .requestMatchers(PUT, "/api/v1/**").hasAnyAuthority(ADMIN_UPDATE.name(), USER_UPDATE.name())
        .requestMatchers(DELETE, "/api/v1/**").hasAnyAuthority(ADMIN_DELETE.name(), USER_DELETE.name())
